...
window.db
  ...
  .stores({
    items:
      "++id, item_id, nav_index, source_platform, last_updated, [item_id+source_platform+last_updated]",
    uploads: "++id",
    nav: "++id, tab_id, session",
    settings: "key",
  })
...
window.zeeschuimer = {
  ...
  init: async function () {
    ...
    setInterval(async function () {
      ...
      for (const module in zeeschuimer.modules) {
        ...
        if (
          is_enabled.hasOwnProperty(enabled_key) &&
          !!parseInt(is_enabled[enabled_key])
        ) {
          enabled.push(module);
        }
      }
  ...
  listener: function (details) {
    ...
    let eligible_modules = Object.fromEntries(
      Object.entries(window.zeeschuimer.modules).filter((entry) => {
        return possible_source_domains.some((domain) =>
          domain.endsWith(entry[1]["domain"].toLowerCase()),
        );
      }),
    );
    ...
    filter.onstop = async (event) => {
      ...
      for (const module_id in eligible_modules) {
        ...
        let module_enabled =
          await browser.storage.local.get(module_enabled_key);
        module_enabled =
          module_enabled.hasOwnProperty(module_enabled_key) &&
          !!parseInt(module_enabled[module_enabled_key]);

        if (module_enabled) {
          enabled_modules.push(module_id);
        }
      }
      await zeeschuimer.parse_request(
        full_response,
        origin_url,
        document_url,
        details.tabId,
  ...
  parse_request: async function (
    ...
    let nav_index = await db.nav
      .where({ tab_id: tabId, session: this.session })
      .first();
    if (!nav_index) {
    ...
    nav_index =
      nav_index.session + ":" + nav_index.tab_id + ":" + nav_index.index;

    const duplicate_behavior_key = "zs-duplicate-behavior";
    const duplicate_behavior = await browser.storage.local.get(
      duplicate_behavior_key,
    );
    let action_on_duplicate =
      duplicate_behavior[duplicate_behavior_key] || "insert";
    ...
    if (!["insert", "skip", "update", "merge"].includes(action_on_duplicate)) {
      console.warn(
        "Invalid global duplicate behavior setting",
        action_on_duplicate,
        '; using default "insert" behavior',
      );
    ...
    for (let module_id in this.modules) {
      ...
      item_list = this.modules[module_id].callback(
        response,
        origin_url,
        document_url,
      );
        ...
        await Promise.all(
          item_list.map(async (item) => {
            ...
            await db.transaction("rw", db.items, async () => {
              ...
              const existing_item_current_nav = await db.items
                .where({
                  item_id: item_id,
                  nav_index: nav_index,
              ...
              if (existing_item_current_nav) {
                ...
                if (
                  module &&
                  typeof module.overwrite_partial === "function" &&
                  (await module.overwrite_partial(
                    item,
                    existing_item_current_nav,
                  ))
                ) {
              ...
              if (action === "update") {
                ...
                await db.items.update(target_item.id, {
                  ...
                  item_id: item_id,
                  timestamp_collected:
                    target_item.timestamp_collected || Date.now(),
                  last_updated: Date.now(),
                  source_platform: module_id,
                  source_platform_url: origin_url,
                  source_url: document_url,
  ...
  nav_handler: async function (tabId) {
    ...
    let nav = await db.nav
      .where({ session: this.session, tab_id: tabId })
      .first();
    if (!nav) {
